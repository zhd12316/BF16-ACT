#!/usr/bin/env python3
"""
Compare Verilog softmax output vs Python golden (bf16).

- Verilog output: softmax_out_tb.bin
- Python golden:  safe_softmax_bf16.bin
...
- Row-sum checks (~1) for rows that are fully finite in each file
"""
...
TB_FILE = os.path.join(DATA_DIR, 'softmax_out_tb.bin')
GD_FILE = os.path.join(DATA_DIR, 'safe_softmax_bf16.bin')
...
def torch_bf16_to_uint16(tensor: torch.Tensor) -> np.ndarray:
    ...
    Matches generate_golden_outputs.py.
    """
...
def main():
    ...
    # Row-wise relative L2 error for rows [20..25] (0-based)
    if rows is not None:
        ...
        print('Row-wise relative L2 error for rows [20..25] (0-based):')
        for r in range(20, 26):
            ...
            den = np.linalg.norm(row_gd)
            if den == 0.0:
                rel = 0.0 if num == 0.0 else float('inf')
            else:
                rel = num / den
