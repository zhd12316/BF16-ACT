#!/usr/bin/env python3
"""
Compare Verilog LayerNorm output vs Python golden (bf16).

- TB output:   layernorm_out_tb.bin
- Python golden: layer_norm_bf16.bin
...
- Row-level NaN suppression: if a row (size 768) contains any NaN in either TB or GD, all comparisons in that row are ignored for mismatch counting.
- No per-file row-sum checks (only softmax needs ~1 row-sums)
"""
...
TB_FILE = os.path.join(DATA_DIR, 'layernorm_out_tb.bin')
GD_FILE = os.path.join(DATA_DIR, 'layer_norm_bf16.bin')
...
def main():
    ...
    # Row-level NaN suppression: if any NaN appears in a row (in TB or GD),
    # ignore all comparisons within that row from mismatch counting.
    suppressed_nan_rows = 0
    if (tb32_np.size % 768) == 0:
        _rows = tb32_np.size // 768
        tb_row_nan = np.isnan(tb32_np).reshape(_rows, 768).any(axis=1)
        gd_row_nan = np.isnan(gd32_np).reshape(_rows, 768).any(axis=1)
        nan_rows_mask = (tb_row_nan | gd_row_nan)
        if np.any(nan_rows_mask):
            row_mask_1d = np.repeat(nan_rows_mask, 768)
            suppressed_nan_rows = int(np.count_nonzero(mismatches & row_mask_1d))
            mismatches[row_mask_1d] = False
    ...
    if suppressed_nan_rows > 0:
        print(f'  (ignored {suppressed_nan_rows} positions from rows containing any NaN)')
    ...
    # Row-sum checks removed per request; keep only error comparisons
    ...
    # Row-wise relative L2 error for rows [26..31] (0-based)
    if rows is not None:
        ...
        print('Row-wise relative L2 error for rows [26..31] (0-based):')
